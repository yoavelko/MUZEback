...
exports.setReqList = async (req, res) => {
    try {
        ...
        const result = await Muze.updateOne({}, { $set: { reqList: tracks } });
        ...
        return res.status(200).json({
            message: 'Req-list replaced successfully',
            updatedReqList: tracks,
        });
    } catch (error) {
        console.error('Error details:', error);
        return res.status(500).json({ error: 'Error setting new req-list.', details: error.message });
    }
};
